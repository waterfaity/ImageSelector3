...
public class ShowFolderAdapter extends BaseAdapter {
    ...
    public ShowFolderAdapter(Context mContext, ArrayList<SearchFolderBean> mData) {
        ...
        this.mData = mData;
        mImgWith = (int) (mContext.getResources().getDisplayMetrics().density * 80);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Picasso.with(mContext).load(new File(searchFolderBean.getFirstImgPath())).centerCrop().resize(mImgWith, mImgWith).into(viewHolder.imageView);
        if (lastPos == position) {
